  ...
  useEffect(() => {
    const fetchWeeklyBookings = async () => {
      ...
      try {
        ...
        // Assuming bookingsResponse.data is BookingsByDate[]
        const bookingsByDate: BookingsByDate[] = bookingsResponse?.data || [];
        const allFlatBookings: SlotBooking[] = bookingsByDate.flatMap(
          (day) => day.bookings_details
        );

        const weekDates = getCurrentWeekDates();

        const currentWeekBookings = allFlatBookings.filter((booking) => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Manage your bowling center's operating hours
              </CardDescription>
            </div>
                    ...
                    <div className="flex items-center space-x-8">
                      {editingHours ? (
                        ...
                        <div className="text-gray-600">
                          {formatTimeForInput(day.startTime)} -{" "}
                          {formatTimeForInput(day.endTime)}
                        </div>
                      )}
                    </div>
                  )}
